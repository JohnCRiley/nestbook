...
## 2026-09-19 — knowledge file: Settings tabs, Billing add-ons, new Channel Manager section
- `help-bot-knowledge.md` restructured: §1 nav now lists **[Channel Manager]** and states that exactly TWO nav items are genuinely removed (Charges, Channel Manager — the latter hidden on Free too); §8 rewritten as the real 5-tab Settings (commit 22ef177); §9 Billing now documents the add-on cards (Bar & Charges = Pro only; Channel Manager = Pro add-on / Multi "Included" badge) + subscription card + Manage Subscription; **new §10 Channel Manager** (why/how, page walkthrough, 4-step wizard, Airbnb OAuth, what it does NOT do, troubleshooting = check the page then support, never diagnose/never lead with Disconnect). Old §10–14 renumbered **§11–15**; "gaps" section is now **§15**, and helpChat.js's prompt reference was updated 14→15. Any future doc/prompt mentioning "Section 14 (gaps)" is stale.
- `helpChat.js`: `loadUserContext` now selects `u.has_channel_manager_addon`; the account-context block adds `- Channel Manager add-on: yes/no` (with a "(not needed — included on Multi)" suffix when plan = multi, so Multi's flag=0 isn't read as "doesn't have it").
- Verified by running the real route with the Anthropic call mocked: Pro w/o add-on → "no", Pro with add-on → "yes", Multi → "no (not needed — …included automatically)"; knowledge served includes §10/five tabs/Section 15 ref. NOT tested with a real model reply (no ANTHROPIC_API_KEY locally).
- Facts confirmed while writing (don't re-check): the owner UI's Airbnb flow is connect-only (OAuth link + callback; no owner listing-mapping/activate screen — routes in properties.js are only adapters/channels/create/check-readiness/activate/airbnb connection-link/deactivate/delete, and the wizard has no Airbnb branch); Recent Activity's Photos/Description/Facilities/Contact lines refer to Channex's own property record (not used by NestBook; John confirmed) — content does NOT reach OTAs; "Payment & Deposit Manager" is the WP Deposit & Balance panel in Settings → Booking & Availability (not a Billing control); Property Info Sheet is its own nav item; Guest Access in Settings is WP-only (SC has per-unit Access & Arrival).
