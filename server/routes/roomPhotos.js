...
roomPhotosRouter.post('/:roomId/photos', upload.single('photo'), async (req, res) => {
  try {
    ...
    // Optimise: resize to max 1200px, convert to JPEG quality 85
    ...
    const result = db.prepare(
      `INSERT INTO room_photos (room_id, property_id, filename, display_order) VALUES (?, ?, ?, ?)`
    ).run(roomId, room.property_id, req.file.filename, nextOrder);
