...
import { ROOM_AMENITIES, PROPERTY_AMENITIES, parseAmenityKeys } from '../utils/amenityCatalog.js';
...
function fmtAmenity(str) {
  return AMENITY_LABELS[String(str).toLowerCase()] ?? (String(str).charAt(0).toUpperCase() + String(str).slice(1));
}

// wifi -> "page.amenityWifi", aircon -> "page.amenityAircon". Returns null for
// a free-text amenity outside AMENITY_LABELS (title-cased, not translated).
function amenityI18nKey(str) {
  const k = String(str).toLowerCase();
  return AMENITY_LABELS[k]
    ? 'page.amenity' + k.charAt(0).toUpperCase() + k.slice(1)
    : null;
}

// One amenity chip, routed through the page's data-i18n pipeline when the
// value is a known amenity so it translates on a language switch like
// everything else; a genuinely custom free-text amenity falls back to a
// title-cased English-ish label with no translation.
function amenityChip(str, cls = 'ws-amenity') {
  const key = amenityI18nKey(str);
  const label = esc(fmtAmenity(str));
  return key
    ? `<span class="${cls}" data-i18n="${key}">${label}</span>`
    : `<span class="${cls}">${label}</span>`;
}

// snake_case catalog key -> the PascalCase suffix used by this page's own
// I18N dict below (page.amenityRoom<Suffix> / page.amenityProperty<Suffix>) —
// deliberately a SEPARATE i18n namespace from the free-text AMENITY_LABELS
// above, since the two are independent systems (structured checklist vs.
// free-text note) that happen to share some English words.
function amenityKeyToSuffix(key) {
  return key.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join('');
}

// One structured-amenity chip (Channex Slice C prerequisite — see
// docs/completed/channex-structured-amenities.md). `scope` is 'Room' or
// 'Property', selecting which catalog + i18n namespace to use. Guest-icon
// image instead of the Tabler font, matching the owner-facing picker so the
// same amenity looks identical in Settings and on this page.
function structuredAmenityChip(key, catalog, scope, cls = 'ws-amenity') {
  const entry = catalog.find(a => a.key === key);
  if (!entry) return '';
  const i18nKey = `page.amenity${scope}${amenityKeyToSuffix(key)}`;
  // Pre-hydration fallback (applyLang() overwrites this on every load,
  // English included, same as every other data-i18n element on this page —
  // but a no-JS/crawler render still deserves a readable label).
  const fallback = key.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
  return `<span class="${cls}"><img src="/images/guest-icons/${esc(entry.icon)}.png" width="16" height="16" alt="" style="vertical-align:middle;margin-right:3px;"><span data-i18n="${i18nKey}">${esc(fallback)}</span></span>`;
}

// Sample-data room descriptions are system-generated by
// server/utils/seedSampleData.js and keyed to room type. On the guest-facing
// page they translate live like the rest of the UI. The English strings here
// AND the page.sampleDesc* values in the I18N dict below must stay identical
// to SAMPLE_CONTENT.*.wp.*.desc in seedSampleData.js.
const SAMPLE_DESC = {
  double:      { key: 'page.sampleDescBedrooms',     en: 'Comfortable bedrooms with plenty of natural light, ready to welcome your guests.' },
  living_room: { key: 'page.sampleDescLivingSpaces', en: 'A warm kitchen and living area where guests can relax and feel at home.' },
  bathroom:    { key: 'page.sampleDescBathroom',     en: "A clean, well-appointed bathroom for your guests' stay." },
};

// <p> for a room/unit description. A system-seeded sample-data description
// (is_sample_data + a type SAMPLE_DESC knows + a non-empty stored value) is
// routed through data-i18n so it translates on a page-language switch; a real
// owner's own description is user content and is rendered verbatim, never
// translated. (Edge case: if an owner edits a still-flagged sample room's
// description to custom text, the preview shows the canonical caption until
// they clear it or delete the sample data — acceptable for preview content.)
function descBlock(room, cls = 'ws-desc') {
  if (room?.is_sample_data && room.description && SAMPLE_DESC[room.type]) {
    const d = SAMPLE_DESC[room.type];
    return `<p class="${cls}" data-i18n="${d.key}">${esc(d.en)}</p>`;
  }
  return room?.description ? `<p class="${cls}">${esc(room.description)}</p>` : '';
}
...
function wpAlternatingShowcase(rooms, photosByRoom, palette) {
  ...
  const rows = rooms.map((room, index) => {
    ...
    const structuredKeys = parseAmenityKeys(room.structured_amenities);
    ...
    const sleepsHtml   = isBedroom && room.capacity
      ...
      : '';
    const amenityChips = structuredKeys.map(k => structuredAmenityChip(k, ROOM_AMENITIES, 'Room')).join('')
      + amenities.map(a => amenityChip(a)).join('');
    const descHtml     = descBlock(room);
...
function generateUnitsPage(units, photosByRoom, currSym, isPaidPlan, internalRoomsByUnit, property) {
  ...
  const rows = units.map((unit, index) => {
    ...
    const structuredKeys = parseAmenityKeys(unit.structured_amenities);
...
function showcaseRoomCard(room, palette, photos) {
  ...
  const amenityTags = structuredKeys.map(k => structuredAmenityChip(k, ROOM_AMENITIES, 'Room', 'amenity-tag')).join('')
    + amenities.map(a => amenityChip(a, 'amenity-tag')).join('');
...
function categoryShowcase(catsWithRooms, categoriesById, photosByRoom, currSym, isPaidPlan, property) {
  ...
  const rows = catsWithRooms.map((category, index) => {
    ...
    const structuredKeys = parseAmenityKeys(category.structured_amenities);
    const amenityChipsHtml = structuredKeys.map(k => structuredAmenityChip(k, ROOM_AMENITIES, 'Room')).join('')
      + categoryAmenities.map(a => amenityChip(a)).join('');
...
<section class="at-a-glance">
  ...
  // Structured property amenities (Channex Slice C prerequisite — see
  // docs/completed/channex-structured-amenities.md). Reuses the At a Glance
  // grid styling for visual consistency; a separate section since these are
  // a curated checklist, not the At a Glance quick-facts.
  const propertyAmenityKeys = parseAmenityKeys(property.amenities);
  const propertyAmenitiesSection = propertyAmenityKeys.length ? `
<section class="at-a-glance property-amenities">
  <div class="section-inner">
    <h2 data-i18n="page.propertyAmenitiesTitle">Amenities</h2>
    <div class="glance-grid">
      ${propertyAmenityKeys.map(key => {
        const entry = PROPERTY_AMENITIES.find(a => a.key === key);
        if (!entry) return '';
        const i18nKey = `page.amenityProperty${amenityKeyToSuffix(key)}`;
        const fallback = key.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
        return `
      <div class="glance-item">
        <img src="/images/guest-icons/${esc(entry.icon)}.png" width="18" height="18" alt="" style="vertical-align:middle;">
        <span data-i18n="${i18nKey}">${esc(fallback)}</span>
      </div>`;
      }).join('')}
    </div>
  </div>
...
<section class="rooms">
  ...
  // Enforcement (bookings.js DELETE) blocks cancellation once
  // daysUntil <= cancellation_days, so free cancellation applies strictly
  // *more than* N days before arrival.
  const cancellationHtml = cancellationDays > 0
    ? `<span data-i18n-n="page.freeCancellation" data-n="${esc(String(cancellationDays))}">Free cancellation more than ${esc(String(cancellationDays))} days before arrival</span>`
    : `<span data-i18n="page.flexibleCancellation">Flexible cancellation — contact the owner</span>`;
...
<section class="cta-section">
  <div class="cta-inner">
    ...
    ${bookOrEnquiryBtn}
    <div style="margin-top:16px;display:flex;justify-content:center;align-items:center;gap:8px;font-size:0.85rem;color:rgba(255,255,255,0.85);">
...
var I18N = {
  en: {
    ...
    "page.freeCancellation":  "Free cancellation more than {n} days before arrival",
    "page.flexibleCancellation": "Flexible cancellation — contact the owner",
    ...
    "page.amenityWifi":               "WiFi",
    "page.amenityEnsuite":            "En-suite",
    "page.amenityBalcony":            "Balcony",
    "page.amenityTerrace":            "Terrace",
    "page.amenityParking":            "Parking",
    "page.amenityMinibar":            "Minibar",
    "page.amenityKitchenette":        "Kitchenette",
    "page.amenityAircon":             "Air Con",
    "page.amenityTv":                 "TV",
    "page.amenitySafe":               "Safe",
    "page.amenityBathtub":            "Bathtub",
    "page.propertyAmenitiesTitle":    "Amenities",
    "page.amenityRoomWifi":               "WiFi",
    "page.amenityRoomTv":                 "TV",
    "page.amenityRoomPrivateBathroom":    "Private bathroom",
    "page.amenityRoomShower":             "Shower",
    "page.amenityRoomAirConditioning":    "Air conditioning",
    "page.amenityRoomHeating":            "Heating",
    "page.amenityRoomMinibar":            "Minibar",
    "page.amenityRoomSafe":               "Safe",
    "page.amenityRoomBalcony":            "Balcony",
    "page.amenityRoomSeaView":            "Sea view",
    "page.amenityRoomHotTub":             "Hot tub",
    "page.amenityRoomWashingMachine":     "Washing machine",
    "page.amenityRoomIron":               "Iron",
    "page.amenityRoomHairdryer":          "Hair dryer",
    "page.amenityRoomFireplace":          "Fireplace",
    "page.amenityRoomSofa":               "Sofa",
    "page.amenityPropertyParking":               "Parking",
    "page.amenityPropertyPool":                  "Swimming pool",
    "page.amenityPropertyPetFriendly":           "Pet-friendly",
    "page.amenityPropertySunTerrace":            "Sun terrace",
    "page.amenityPropertyReception24hr":         "24-hour reception",
    "page.amenityPropertyWifi":                  "WiFi",
    "page.amenityPropertyRestaurant":            "Restaurant",
    "page.amenityPropertyBar":                   "Bar",
    "page.amenityPropertyAirConditioning":       "Air conditioning",
    "page.amenityPropertyNonSmoking":            "Non-smoking rooms",
    "page.amenityPropertyElevator":              "Lift / elevator",
    "page.amenityPropertyLaundry":               "Laundry",
    "page.amenityPropertyWheelchairAccessible":  "Wheelchair accessible",
    "page.amenityPropertyGarden":                "Garden",
    "page.amenityPropertyBbq":                   "BBQ area",
    "page.amenityPropertyBaggageStorage":        "Baggage storage",
    "page.amenityPropertyFirstAid":              "First aid kit",
    "page.amenityPropertyFireExtinguisher":      "Fire extinguisher",
    "page.sampleDescBedrooms":        "Comfortable bedrooms with plenty of natural light, ready to welcome your guests.",
    "page.sampleDescLivingSpaces":    "A warm kitchen and living area where guests can relax and feel at home.",
    "page.sampleDescBathroom":        "A clean, well-appointed bathroom for your guests' stay."
  },
  fr: {
    ...
    "page.freeCancellation":  "Annulation gratuite plus de {n} jours avant l'arrivée",
    "page.flexibleCancellation": "Annulation flexible — contactez le propriétaire",
    ...
    "page.amenityWifi":               "WiFi",
    "page.amenityEnsuite":            "Salle de bains privative",
    "page.amenityBalcony":            "Balcon",
    "page.amenityTerrace":            "Terrasse",
    "page.amenityParking":            "Parking",
    "page.amenityMinibar":            "Minibar",
    "page.amenityKitchenette":        "Kitchenette",
    "page.amenityAircon":             "Climatisation",
    "page.amenityTv":                 "Télévision",
    "page.amenitySafe":               "Coffre-fort",
    "page.amenityBathtub":            "Baignoire",
    "page.propertyAmenitiesTitle":    "Équipements",
    "page.amenityRoomWifi":               "WiFi",
    "page.amenityRoomTv":                 "TV",
    "page.amenityRoomPrivateBathroom":    "Salle de bain privée",
    "page.amenityRoomShower":             "Douche",
    "page.amenityRoomAirConditioning":    "Climatisation",
    "page.amenityRoomHeating":            "Chauffage",
    "page.amenityRoomMinibar":            "Minibar",
    "page.amenityRoomSafe":               "Coffre-fort",
    "page.amenityRoomBalcony":            "Balcon",
    "page.amenityRoomSeaView":            "Vue sur mer",
    "page.amenityRoomHotTub":             "Jacuzzi",
    "page.amenityRoomWashingMachine":     "Lave-linge",
    "page.amenityRoomIron":               "Fer à repasser",
    "page.amenityRoomHairdryer":          "Sèche-cheveux",
    "page.amenityRoomFireplace":          "Cheminée",
    "page.amenityRoomSofa":               "Canapé",
    "page.amenityPropertyParking":               "Parking",
    "page.amenityPropertyPool":                  "Piscine",
    "page.amenityPropertyPetFriendly":           "Animaux acceptés",
    "page.amenityPropertySunTerrace":            "Terrasse ensoleillée",
    "page.amenityPropertyReception24hr":         "Réception 24h/24",
    "page.amenityPropertyWifi":                  "WiFi",
    "page.amenityPropertyRestaurant":            "Restaurant",
    "page.amenityPropertyBar":                   "Bar",
    "page.amenityPropertyAirConditioning":       "Climatisation",
    "page.amenityPropertyNonSmoking":            "Chambres non-fumeurs",
    "page.amenityPropertyElevator":              "Ascenseur",
    "page.amenityPropertyLaundry":               "Blanchisserie",
    "page.amenityPropertyWheelchairAccessible":  "Accès fauteuil roulant",
    "page.amenityPropertyGarden":                "Jardin",
    "page.amenityPropertyBbq":                   "Espace barbecue",
    "page.amenityPropertyBaggageStorage":        "Consigne à bagages",
    "page.amenityPropertyFirstAid":              "Trousse de premiers secours",
    "page.amenityPropertyFireExtinguisher":      "Extincteur",
    "page.sampleDescBedrooms":        "Des chambres confortables et lumineuses, prêtes à accueillir vos invités.",
    "page.sampleDescLivingSpaces":    "Une cuisine chaleureuse et un espace de vie où les invités peuvent se détendre et se sentir chez eux.",
    "page.sampleDescBathroom":        "Une salle de bains propre et bien équipée pour le séjour de vos invités."
  },
  de: {
    ...
    "page.freeCancellation":  "Kostenlose Stornierung bei mehr als {n} Tagen vor der Anreise",
    "page.flexibleCancellation": "Flexible Stornierung — kontaktieren Sie den Gastgeber",
    ...
    "page.amenityWifi":               "WLAN",
    "page.amenityEnsuite":            "Eigenes Bad",
    "page.amenityBalcony":            "Balkon",
    "page.amenityTerrace":            "Terrasse",
    "page.amenityParking":            "Parkplatz",
    "page.amenityMinibar":            "Minibar",
    "page.amenityKitchenette":        "Küchenzeile",
    "page.amenityAircon":             "Klimaanlage",
    "page.amenityTv":                 "Fernseher",
    "page.amenitySafe":               "Safe",
    "page.amenityBathtub":            "Badewanne",
    "page.propertyAmenitiesTitle":    "Ausstattung",
    "page.amenityRoomWifi":               "WLAN",
    "page.amenityRoomTv":                 "TV",
    "page.amenityRoomPrivateBathroom":    "Eigenes Bad",
    "page.amenityRoomShower":             "Dusche",
    "page.amenityRoomAirConditioning":    "Klimaanlage",
    "page.amenityRoomHeating":            "Heizung",
    "page.amenityRoomMinibar":            "Minibar",
    "page.amenityRoomSafe":               "Safe",
    "page.amenityRoomBalcony":            "Balkon",
    "page.amenityRoomSeaView":            "Meerblick",
    "page.amenityRoomHotTub":             "Whirlpool",
    "page.amenityRoomWashingMachine":     "Waschmaschine",
    "page.amenityRoomIron":               "Bügeleisen",
    "page.amenityRoomHairdryer":          "Haartrockner",
    "page.amenityRoomFireplace":          "Kamin",
    "page.amenityRoomSofa":               "Sofa",
    "page.amenityPropertyParking":               "Parkplatz",
    "page.amenityPropertyPool":                  "Schwimmbad",
    "page.amenityPropertyPetFriendly":           "Haustiere erlaubt",
    "page.amenityPropertySunTerrace":            "Sonnenterrasse",
    "page.amenityPropertyReception24hr":         "24-Stunden-Rezeption",
    "page.amenityPropertyWifi":                  "WLAN",
    "page.amenityPropertyRestaurant":            "Restaurant",
    "page.amenityPropertyBar":                   "Bar",
    "page.amenityPropertyAirConditioning":       "Klimaanlage",
    "page.amenityPropertyNonSmoking":            "Nichtraucherzimmer",
    "page.amenityPropertyElevator":              "Aufzug",
    "page.amenityPropertyLaundry":               "Wäscheservice",
    "page.amenityPropertyWheelchairAccessible":  "Rollstuhlgerecht",
    "page.amenityPropertyGarden":                "Garten",
    "page.amenityPropertyBbq":                   "Grillbereich",
    "page.amenityPropertyBaggageStorage":        "Gepäckaufbewahrung",
    "page.amenityPropertyFirstAid":              "Erste-Hilfe-Set",
    "page.amenityPropertyFireExtinguisher":      "Feuerlöscher",
    "page.sampleDescBedrooms":        "Komfortable, lichtdurchflutete Schlafzimmer, bereit für Ihre Gäste.",
    "page.sampleDescLivingSpaces":    "Eine gemütliche Küche und ein Wohnbereich, in dem sich Gäste entspannen und wohlfühlen können.",
    "page.sampleDescBathroom":        "Ein sauberes, gut ausgestattetes Badezimmer für den Aufenthalt Ihrer Gäste."
  },
  es: {
    ...
    "page.freeCancellation":  "Cancelación gratuita con más de {n} días de antelación a la llegada",
    "page.flexibleCancellation": "Cancelación flexible — contacte con el propietario",
    ...
    "page.amenityWifi":               "WiFi",
    "page.amenityEnsuite":            "Baño privado",
    "page.amenityBalcony":            "Balcón",
    "page.amenityTerrace":            "Terraza",
    "page.amenityParking":            "Aparcamiento",
    "page.amenityMinibar":            "Minibar",
    "page.amenityKitchenette":        "Cocina americana",
    "page.amenityAircon":             "Aire acondicionado",
    "page.amenityTv":                 "Televisión",
    "page.amenitySafe":               "Caja fuerte",
    "page.amenityBathtub":            "Bañera",
    "page.propertyAmenitiesTitle":    "Comodidades",
    "page.amenityRoomWifi":               "WiFi",
    "page.amenityRoomTv":                 "TV",
    "page.amenityRoomPrivateBathroom":    "Baño privado",
    "page.amenityRoomShower":             "Ducha",
    "page.amenityRoomAirConditioning":    "Aire acondicionado",
    "page.amenityRoomHeating":            "Calefacción",
    "page.amenityRoomMinibar":            "Minibar",
    "page.amenityRoomSafe":               "Caja fuerte",
    "page.amenityRoomBalcony":            "Balcón",
    "page.amenityRoomSeaView":            "Vista al mar",
    "page.amenityRoomHotTub":             "Bañera de hidromasaje",
    "page.amenityRoomWashingMachine":     "Lavadora",
    "page.amenityRoomIron":               "Plancha",
    "page.amenityRoomHairdryer":          "Secador de pelo",
    "page.amenityRoomFireplace":          "Chimenea",
    "page.amenityRoomSofa":               "Sofá",
    "page.amenityPropertyParking":               "Aparcamiento",
    "page.amenityPropertyPool":                  "Piscina",
    "page.amenityPropertyPetFriendly":           "Admite mascotas",
    "page.amenityPropertySunTerrace":            "Terraza soleada",
    "page.amenityPropertyReception24hr":         "Recepción 24 horas",
    "page.amenityPropertyWifi":                  "WiFi",
    "page.amenityPropertyRestaurant":            "Restaurante",
    "page.amenityPropertyBar":                   "Bar",
    "page.amenityPropertyAirConditioning":       "Aire acondicionado",
    "page.amenityPropertyNonSmoking":            "Habitaciones para no fumadores",
    "page.amenityPropertyElevator":              "Ascensor",
    "page.amenityPropertyLaundry":               "Lavandería",
    "page.amenityPropertyWheelchairAccessible":  "Acceso para sillas de ruedas",
    "page.amenityPropertyGarden":                "Jardín",
    "page.amenityPropertyBbq":                   "Zona de barbacoa",
    "page.amenityPropertyBaggageStorage":        "Custodia de equipaje",
    "page.amenityPropertyFirstAid":              "Botiquín de primeros auxilios",
    "page.amenityPropertyFireExtinguisher":      "Extintor",
    "page.sampleDescBedrooms":        "Dormitorios cómodos y luminosos, listos para recibir a sus huéspedes.",
    "page.sampleDescLivingSpaces":    "Una cocina acogedora y una zona de estar donde los huéspedes pueden relajarse y sentirse como en casa.",
    "page.sampleDescBathroom":        "Un baño limpio y bien equipado para la estancia de sus huéspedes."
  },
  nl: {
    ...
    "page.freeCancellation":  "Gratis annulering meer dan {n} dagen voor aankomst",
    "page.flexibleCancellation": "Flexibele annulering — neem contact op met de eigenaar",
    ...
    "page.amenityWifi":               "Wifi",
    "page.amenityEnsuite":            "Eigen badkamer",
    "page.amenityBalcony":            "Balkon",
    "page.amenityTerrace":            "Terras",
    "page.amenityParking":            "Parkeerplaats",
    "page.amenityMinibar":            "Minibar",
    "page.amenityKitchenette":        "Kitchenette",
    "page.amenityAircon":             "Airconditioning",
    "page.amenityTv":                 "Televisie",
    "page.amenitySafe":               "Kluis",
    "page.amenityBathtub":            "Bad",
    "page.propertyAmenitiesTitle":    "Voorzieningen",
    "page.amenityRoomWifi":               "WiFi",
    "page.amenityRoomTv":                 "TV",
    "page.amenityRoomPrivateBathroom":    "Eigen badkamer",
    "page.amenityRoomShower":             "Douche",
    "page.amenityRoomAirConditioning":    "Airconditioning",
    "page.amenityRoomHeating":            "Verwarming",
    "page.amenityRoomMinibar":            "Minibar",
    "page.amenityRoomSafe":               "Kluis",
    "page.amenityRoomBalcony":            "Balkon",
    "page.amenityRoomSeaView":            "Zeezicht",
    "page.amenityRoomHotTub":             "Bubbelbad",
    "page.amenityRoomWashingMachine":     "Wasmachine",
    "page.amenityRoomIron":               "Strijkijzer",
    "page.amenityRoomHairdryer":          "Föhn",
    "page.amenityRoomFireplace":          "Open haard",
    "page.amenityRoomSofa":               "Bank",
    "page.amenityPropertyParking":               "Parkeergelegenheid",
    "page.amenityPropertyPool":                  "Zwembad",
    "page.amenityPropertyPetFriendly":           "Huisdieren toegestaan",
    "page.amenityPropertySunTerrace":            "Zonneterras",
    "page.amenityPropertyReception24hr":         "24-uurs receptie",
    "page.amenityPropertyWifi":                  "WiFi",
    "page.amenityPropertyRestaurant":            "Restaurant",
    "page.amenityPropertyBar":                   "Bar",
    "page.amenityPropertyAirConditioning":       "Airconditioning",
    "page.amenityPropertyNonSmoking":            "Niet-rokerskamers",
    "page.amenityPropertyElevator":              "Lift",
    "page.amenityPropertyLaundry":               "Wasserij",
    "page.amenityPropertyWheelchairAccessible":  "Rolstoeltoegankelijk",
    "page.amenityPropertyGarden":                "Tuin",
    "page.amenityPropertyBbq":                   "Barbecueplek",
    "page.amenityPropertyBaggageStorage":        "Bagageopslag",
    "page.amenityPropertyFirstAid":              "EHBO-kit",
    "page.amenityPropertyFireExtinguisher":      "Brandblusser",
    "page.sampleDescBedrooms":        "Comfortabele slaapkamers met veel natuurlijk licht, klaar om uw gasten te ontvangen.",
    "page.sampleDescLivingSpaces":    "Een warme keuken en woonruimte waar gasten kunnen ontspannen en zich thuis voelen.",
    "page.sampleDescBathroom":        "Een schone, goed uitgeruste badkamer voor het verblijf van uw gasten."
  }
